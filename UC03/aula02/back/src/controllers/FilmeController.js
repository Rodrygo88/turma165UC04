...
export default class FilmeController{
    ...
    static async listarFilmes(req, res){
        try {
            ...
            if(result.rowCount === 0){
                res.status(404).json({msg: "Nenhum filme no banco de dados."});
                return;
            }
            res.status(200).json({msg:"Filmes encontrados!", filmes: result.rows});
        } catch (error) {
            res.status(500).json({msg: "Erro interno ao listar filmes.", erro: error.message});
        }
    }
    ...
    static async buscarPorId(req, res){
        try {
            ...
            if(result.rowCount === 0){
                res.status(404).json({msg: "Nenhum filme com esse id no banco de dados."});
                return;
            }
            res.status(200).json({msg:"Filme encontrado!", filmes: result.rows[0]});
        } catch (error) {
    ...
    static async criarFilme(req, res){
        try {
            ...
            res.status(500).json({msg: "Erro interno ao cadastrar filme.", erro: error.message});
        }
    }
